    ...
    <div className="space-y-5">
      ...
      {status === "error" && (
        <p className="text-destructive">
          Erreur dans l`&apos;extraction des avis
        </p>
      )}
      {status === "success" && !reviewItems.length && !hasNextPage && (
        <p>Pas encore de commentairest</p>
      )}
      ...
      {reply?.message && (
        <div className="ms-10 mt-2.5 space-y-1 border-t pt-2.5">
          <div className="flex items-center gap-2">
            ...
            <Image
              src={logo}
              alt="alibia Store logo"
              width={24}
              height={24}
              className="size-5"
            />
            <span className="font-bold">ALIBIA TEAMS</span>
          </div>
...
function MediaAttachment({ media }: MediaAttachmentProps) {
  ...
  return (
    <span className="text-destructive">Type de support non pris en charge</span>
  );
}
